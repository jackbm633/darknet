...
CC=$(TOOLCHAIN_PATH)/cross/bin/i686-aldebaran-linux-gnu-gcc
CPP=$(TOOLCHAIN_PATH)/cross/bin/i686-aldebaran-linux-gnu-g++
NVCC=nvcc 
AR=$(TOOLCHAIN_PATH)/cross/bin/i686-aldebaran-linux-gnu-ar
ARFLAGS=rcs
# Added some flags to optimise for the Atom processor in the Naos.
OPTS= -O3 -march=atom
LDFLAGS= -lm -pthread 
COMMON=
CFLAGS=-Wall -Wfatal-errors -Wno-unused-result -Wno-unknown-pragmas

### Uncomment to build locally on your PC without cross toolchain
# CC=clang
# CPP=clang++
# NVCC=nvcc 
# AR=ar
...
$(LIBNAMESO): $(OBJS) src/yolo_v2_class.hpp src/yolo_v2_class.cpp
	$(CPP) -shared -std=gnu++0x -fvisibility=hidden -DYOLODLL_EXPORTS $(COMMON) $(CFLAGS) $(OBJS) src/yolo_v2_class.cpp -o $@ $(LDFLAGS)
	
$(APPNAMESO): $(LIBNAMESO) src/yolo_v2_class.hpp src/yolo_console_dll.cpp
	$(CPP) -B~/nbites/lib/atomtoolchain/cross/i686-aldebaran-linux-gnu/bin/ -std=c++0x $(COMMON) $(CFLAGS) -o $@ src/yolo_console_dll.cpp $(LDFLAGS) -L ./ -l:$(LIBNAMESO)
endif
